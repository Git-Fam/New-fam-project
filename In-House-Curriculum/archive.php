...
foreach ($users as $user) {
    ...
    foreach ($user_meta as $meta_key => $meta_value) {
        if (preg_match('/^(env|VAL|INIT|div|responsive|JQ|LP|MiniLP|Sass|React|Java|SQL|Design|SEO|Form|FAM|test|JS|wordpress|jstqb|verify|node|Spec|sass|test|cl)/i', $meta_key)) {
        ...
        <div class="road-chat">
            <div class="C_chat-content">
                ...
                <div class="timeline-wrap">
                        <div class="timeline">
                            <?php
                            $timeline_limit = 8; // 流す件数の上限

                            // 全スタッフを対象に完了データを集める
                            $timeline_users = get_users();

                            // タグスラッグ→投稿タイトルの対応表を一度だけ作る（クエリ削減）
                            $tag_title_map = [];
                            $all_tagged_posts = get_posts([
                                'post_type'      => 'post',
                                'posts_per_page' => -1,
                                'fields'         => 'ids',
                            ]);
                            ...
                            foreach ($timeline_users as $tuser) {
                                $tuser_id = $tuser->ID;

                                // 管理者などを除外（既存の is_valid_role を流用）
                                if (!is_valid_role($tuser_id)) continue;

                                $tuser_name = $tuser->display_name;
                                $tuser_meta = get_user_meta($tuser_id);

                                foreach ($tuser_meta as $meta_key => $meta_value) {
                                    // 進捗キーのみ対象／_date キーは除外
                                    if (
                                        preg_match('/^(env|VAL|INIT|div|responsive|JQ|LP|MiniLP|Sass|React|Java|SQL|Design|SEO|Form|FAM|test|JS|wordpress|jstqb|verify|node|Spec|sass|test|cl)/i', $meta_key) &&
                                        substr($meta_key, -5) !== '_date'
                                    ) {
                                        if ($meta_value[0] == '100') {
                                            $date_meta_key = $meta_key . '_date';
                                            $completion_date = get_user_meta($tuser_id, $date_meta_key, true);
                                            if (!$completion_date) continue; // 完了日が無いものは流さない

                                            $task_title = isset($tag_title_map[$meta_key]) ? $tag_title_map[$meta_key] : $meta_key;
                            ...
                            $completions = array_slice($completions, 0, $timeline_limit);

                            // 出力
                            if (!empty($completions)) {
                                foreach ($completions as $c) {
                                    echo '<div class="timeline-item">';
                                    echo '<h3>' . esc_html($c['user_name']) . 'さんが<br>' . esc_html($c['post_title']) . ' を完了しました！</h3>';
                                    echo '</div>';
                                }
                            } else {
                                echo '<div class="timeline-item timeline-item--empty">';
                                echo '<h3>まだ完了報告はありません</h3>';
                                echo '</div>';
                            }
                            ?>
                        </div>
