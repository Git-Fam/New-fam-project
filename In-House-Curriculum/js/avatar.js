$(function () {
  ...
  $('input.category-tag__item--wrap').on('change', function () {
    ...
    var isAvatar = $controlWrap.hasClass('active') && $controlWrap.prevAll('.control__list__wrap').length === 0;

    console.log('isAvatar:', isAvatar);
    console.log('$controlWrap.hasClass("active"):', $controlWrap.hasClass('active'));
    console.log('$controlWrap.prevAll(".control__list__wrap").length:', $controlWrap.prevAll('.control__list__wrap').length);
    ...
    if (isAvatar) {
      ...
      var $avatarCategories = $('.control__list__wrap').first().find('.control__category-tag__list');
      var isAvatarCategory = false;
      $avatarCategories.each(function () {
        var avatarCategoryName = $(this).find('input').first().attr('name').replace('selected_items-', '');
      ...
      if ($itemElement.length === 0) {
        // アイテム要素が存在しない場合は作成
        $('.display__character').append('<img class="selected_items-item selected_items-' + category + '" src="' + thumbnailUrl + '">');
      ...
      if (currentSelectedAvatar) {
        ...
        if (avatarItemStyles && avatarItemStyles[value.split('-').pop()]) {
          console.log('Applying avatar-specific style:', avatarItemStyles[value.split('-').pop()]);
  ...
  $('input.category-tag__item--wrap').on('change', function () {
    ...
    console.log('$(this).siblings(".nothing-item").hasClass("active"):', $(this).siblings('.nothing-item').hasClass('active'));
  ...
  function updateCharacterDisplay() {
    ...
    if (selectedAvatar) {
      var avatarThumbnail = $('input[value="' + selectedAvatar + '"]').closest('li').find('img').attr('src');
      var avatarAspectRatio = $('input[value="' + selectedAvatar + '"]').closest('li').find('.aspect-ratio').text();
      var avatarStyle = $('input[value="' + selectedAvatar + '"]').closest('li').find('.avatar-style').text();
    ...
    $.each(checkedItems, function (category, value) {
      ...
      var $controlWrap = $input.closest('.control__list__wrap');
      var isItem = !($controlWrap.hasClass('active') && $controlWrap.prevAll('.control__list__wrap').length === 0);
      ...
      if ($itemElement.length === 0) {
        // アイテム要素が存在しない場合は作成
        $('.display__character').append('<img class="selected_items-item selected_items-' + category + '" src="' + itemThumbnail + '">');
        $itemElement = $('.selected_items-' + category);
      } else {
      ...
      if (currentSelectedAvatar) {
        ...
        if (avatarItemStyles && avatarItemStyles[itemId]) {
          console.log('Applying avatar-specific style:', avatarItemStyles[itemId]);
  ...
  $(document).ready(function () {
    ...
    // デフォルトのアバター情報を設定
    var defaultAvatarInput = $('input[value*="normal-7376"]').first();
    if (defaultAvatarInput.length > 0) {
      currentSelectedAvatar = {
        id: '7376',
        value: 'normal-7376',
        thumbnail: defaultAvatarInput.closest('li').find('img').attr('src'),
        aspectRatio: defaultAvatarInput.closest('li').find('.aspect-ratio').text(),
