jQuery(function () {
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          if (response.success) {
            ...
            // ★ 押した瞬間だけ：メッセージに「コイン」が含まれるときだけ演出
            if (
              response.data.message &&
              response.data.message.indexOf("コイン") !== -1
            ) {
              celebrateCoin();
            }
          } else {
  ...
  function celebrateCoin() {
    $(".coin-counter").each(function () {
      var $cc = $(this);

      // 跳ね演出（celebrate クラスで発火）
      $cc.removeClass("celebrate");
      void this.offsetWidth;
      $cc.addClass("celebrate");

      // ＋3coin フロート
      $cc.find(".coin-float").remove();
      var $float = $('<span class="coin-float">＋3 coin</span>');
      $cc.append($float);
      void $float[0].offsetWidth;
      $float.addClass("run");
      setTimeout(function () {
        $float.remove();
      }, 1500);

       // ★ 演出後、C_reaction ブロック全体をグレーに
  setTimeout(function () {
    $(".C_reaction").addClass("spent");
  }, 1600);
    });
  }
  ...
  function updateLikeInfo(like_count_today = null) {
    ...
    if (like_count_today == 5) {
      $(".coin-counter").addClass("get"); // 状態フラグ（必要なら）
      $(".C_reaction").addClass("spent"); // ブロック全体グレー
    } else {
      $(".coin-counter").removeClass("get");
      $(".C_reaction").removeClass("spent");
    }
  }
