      ...
      fetch(ajax_data.ajax_url, {
        ...
        .then((res) => {
          ...
          return res.text().then((text) => {
            try {
              ...
              // レスポンスが空でない場合、成功とみなす（実際には更新されている可能性がある）
              if (text.trim().length > 0) {
                // 成功として処理を続行
                return { success: true, data: "進捗を更新しました" };
              }

              throw new Error(
                "サーバーからの応答がJSON形式ではありません: " + e.message
              );
            }
          });
        })
        .then((data) => {
          if (data && data.success) {
            ...
            btn.disabled = false;
            const errorMsg =
              data && data.data ? data.data : "不明なエラーが発生しました";
            alert("エラー: " + errorMsg);
          }
        })
        .catch((err) => {
